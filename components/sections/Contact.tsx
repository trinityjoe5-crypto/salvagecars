    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <div data-reveal data-reveal-delay="80" className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-12">
          ...
          >
            {t.contact.heading1} <span style={{ color: '#E8870C' }}>{t.contact.headingAccent}</span>
          </h2>
        </div>
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-4">
              {contactItems.map(({ label, value, href, icon }) => (
                <div key={label} className="flex items-center gap-4 p-4 bg-surface rounded-xl border border-white/6">
                  ...
                  <div>
                    <p className="text-[11px] font-semibold uppercase tracking-widest text-white/30 mb-0.5" style={{ fontFamily: 'var(--font-barlow), sans-serif' }}>
                      {label}
                    </p>
                    {href ? (
                      <a href={href} className="text-[15px] font-medium text-white hover:text-amber transition-colors duration-150" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>
                        {value}
                      </a>
                    ) : (
                      <p className="text-[15px] text-white/60" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>{value}</p>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="bg-surface rounded-2xl p-7 sm:p-8 border border-white/6">
            {sent ? (
              <div className="flex flex-col items-center justify-center text-center py-10 gap-4">
                ...
                <h3 className="text-[22px] font-bold text-white" style={{ fontFamily: 'var(--font-barlow), sans-serif' }}>
                  {t.contact.successTitle}
                </h3>
                <p className="text-[14px] text-white/50" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>
              ...
              <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
                <h3 className="text-[20px] font-bold text-white mb-1" style={{ fontFamily: 'var(--font-barlow), sans-serif', letterSpacing: '-0.01em' }}>
                  {t.contact.formTitle}
                </h3>

                <div className="flex flex-col gap-1.5">
                  <label htmlFor="contact-name" className="text-[12px] font-semibold uppercase tracking-widest text-white/40" style={{ fontFamily: 'var(--font-barlow), sans-serif' }}>
                    {t.contact.nameLabel}
                  </label>
                  <input
                    ...
                    className="w-full px-4 py-3 bg-graphite-600 text-white text-[15px] rounded-xl border border-white/10 placeholder:text-white/25 focus:outline-none focus:border-amber/50 focus:ring-1 focus:ring-amber/30 transition-[border-color] duration-150"
                    style={{ fontFamily: 'var(--font-figtree), sans-serif' }}
                  />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label htmlFor="contact-phone" className="text-[12px] font-semibold uppercase tracking-widest text-white/40" style={{ fontFamily: 'var(--font-barlow), sans-serif' }}>
                ...
                <div className="flex flex-col gap-1.5">
                  <label htmlFor="contact-message" className="text-[12px] font-semibold uppercase tracking-widest text-white/40" style={{ fontFamily: 'var(--font-barlow), sans-serif' }}>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-graphite-600 text-white text-[15px] rounded-xl border border-white/10 placeholder:text-white/25 resize-none focus:outline-none focus:border-amber/50 focus:ring-1 focus:ring-amber/30 transition-[border-color] duration-150"
                    style={{ fontFamily: 'var(--font-figtree), sans-serif' }}
                  />
                </div>
