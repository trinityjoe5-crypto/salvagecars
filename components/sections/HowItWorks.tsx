...
export function HowItWorks() {
  ...
  const badges = [
    { label: t.howItWorks.badge1label, sub: t.howItWorks.badge1sub },
    { label: t.howItWorks.badge2label, sub: t.howItWorks.badge2sub },
    { label: t.howItWorks.badge3label, sub: t.howItWorks.badge3sub },
  ]

  return (
    <section id="vykup" className="relative bg-graphite py-20 sm:py-28" aria-labelledby="how-it-works-heading">
      ...
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        ...
        <div data-reveal data-reveal-delay="40" className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-14">
          <h2 id="how-it-works-heading" style={{ fontFamily: 'var(--font-barlow), sans-serif', fontWeight: 700, fontSize: 'clamp(2rem, 4vw, 2.75rem)', letterSpacing: '-0.025em', color: '#ffffff', lineHeight: 1.1 }}>
            {t.howItWorks.heading1}{' '}
            <span style={{ color: '#E8870C' }}>{t.howItWorks.headingAccent}</span>
            <br />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {steps.map((step, i) => (
            ...
            >
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-10 -right-3 w-6 h-[2px] bg-white/10" aria-hidden="true" />
              )}
              <span className="block text-[40px] font-black leading-none mb-5" style={{ fontFamily: 'var(--font-barlow), sans-serif', color: '#E8870C', letterSpacing: '-0.04em', opacity: 0.9 }} aria-hidden="true">
                {step.number}
              </span>
              <h3 className="text-[18px] font-bold text-white mb-3 leading-tight" style={{ fontFamily: 'var(--font-barlow), sans-serif', letterSpacing: '-0.01em' }}>
                {step.title}
              </h3>
              <p className="text-[14px] text-white/50 leading-[1.7]" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>
                {step.description}
              </p>
            </div>
          ))}
        ...
        <div className="mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4">
          {badges.map(({ label, sub }) => (
            <div key={label} className="flex items-start gap-3 px-5 py-4 bg-surface/50 rounded-xl border border-white/5">
              ...
              <div>
                <p className="text-[14px] font-semibold text-white" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>{label}</p>
                <p className="text-[12px] text-white/40 mt-0.5" style={{ fontFamily: 'var(--font-figtree), sans-serif' }}>{sub}</p>
              </div>
            </div>
          ))}
        </div>
